...
function Add_User({ setshowAddUser, workspace }) {
    ...
    useEffect(() => {
        ...
        .then((res) => {
            const workspaceMemberIds = workspace.members.map(member => member._id.toString());
        ...
        <div className="bg-white p-4 rounded-md w-96 max-w-full relative">
            ...
            <div className="users-list flex flex-col gap-2 mb-16 max-h-80 overflow-auto">
                {filteredUsers.map((usr) => {
                    const isSelected = selectedUsers.some(selected => selected._id === usr._id);
                    return (
                        <div 
                            key={usr._id} 
                            onClick={() => toggleUserSelection(usr)}
                            className={`user cursor-pointer p-2 flex gap-2 items-center rounded ${isSelected ? 'bg-slate-300' : 'hover:bg-slate-200'}`}
                        >
            ...
            <button onClick={()=>{handleAddUser()}} className="absolute bottom-4 left-1/2 transform -translate-x-1/2 px-4 py-2 bg-gray-700 text-white rounded-md">
                Add Collaborators
            </button>
        </div>
    );
