...
function Task(props) {
  ...
  return (
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-5">
        ...
        {task.assignedBy === task.assignedTo ? (
          <>
            <Link to={`/task/edit-task/${task._id}`} className="ri-edit-2-fill"></Link>
            <Link to={`/task/delete-task/${task._id}`} className="ri-delete-bin-6-fill"></Link>
          </>
        ) : (
          "Assigned"
        )}
    ...
    <p className="text-md font-mono opacity-50 -mt-2 mb-2">{task.description}</p>
  
    {task.status === "in-complete" && (
      <p className="text-red-500 font-semibold text-sm">⚠️ Task Incomplete or Deadline Missed</p>
    )}
